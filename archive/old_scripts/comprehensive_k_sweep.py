...
def main():
    ...
    parser.add_argument(
        ...
        default="batch",
        help="Generation mode: batch (uses ring-buffer KV, default) or single (uses concatenation KV, not zero-copy)",
    )
    ...
    system_info["mode"] = args.mode

    # Warn if single mode is used (does not use ring-buffer KV)
    if args.mode == "single":
        logger.warning(
            "WARNING: Single mode does NOT use ring-buffer KV cache. "
            "It uses concatenation-based KV append (not zero-copy). "
            "For zero-copy benchmarks, use --mode batch (default)."
        )
        print(
            "[WARNING] Single mode selected: This path uses torch.cat() for KV cache, "
            "NOT the zero-copy ring buffer. Use --mode batch for zero-copy experiments.",
            flush=True,
        )
